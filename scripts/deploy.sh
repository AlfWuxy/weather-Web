...
remote_exec "systemctl stop case-weather || true; systemctl stop case-weather-dispatch.timer || true; systemctl stop case-weather-risk-precompute.timer || true; systemctl stop case-weather-risk-precompute.service || true"
...
EnvironmentFile=$PROJECT_DIR/.env
Environment=PYTHONUNBUFFERED=1
ExecStart=$PROJECT_DIR/scripts/community_risk_precompute.sh
EOF

cat > /etc/systemd/system/case-weather-risk-precompute.timer << 'EOF'
[Unit]
Description=Case Weather - precompute community risk every 20 minutes
...
echo ""
echo "步骤8.2: 启动社区风险预热定时器..."
remote_exec "systemctl daemon-reload && systemctl enable --now case-weather-risk-precompute.timer && systemctl status --no-pager case-weather-risk-precompute.timer || true"
