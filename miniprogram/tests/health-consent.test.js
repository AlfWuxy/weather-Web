...
test('老人创建页首次 onLoad 和 onShow 共用守卫后仍显示表单', async () => {
  resetSession('elder-create-first-show-token');
  const requests = [];
  requestImpl = async (options) => {
    requests.push(options.path);
    return {
      required_health_consent_version: 'health-create-v1',
      health_consent_current: true,
    };
  };
  assert.equal(await careSession.ensureHealthConsent(), true);

  delete require.cache[require.resolve('../pages/elder-edit/index')];
  pageDefinition = null;
  require('../pages/elder-edit/index');
  const page = makePage(pageDefinition);
  currentPages = [{ route: 'pages/elders/index' }, page];

  const initialLoad = page.onLoad.call(page, { mode: 'create' });
  const initialShow = page.onShow.call(page);
  await Promise.all([initialLoad, initialShow]);

  assert.equal(page.data.loading, false);
  assert.equal(page.data.contextReady, true);
  assert.equal(page.data.loadError, '');
  assert.equal(page._healthConsentLoadedOnce, true);
  assert.deepEqual(requests, ['/mp/api/v1/health-consent']);
...
test('健康同意核验网络失败通知页面持久报错且重试后才能加载', async () => {
  resetSession('health-guard-network-token');
  const networkError = Object.assign(new Error('request:fail timeout'), {
    statusCode: 503,
    code: 'http_503',
  });
  let guardErrors = 0;
  ...
  const page = {
    _unloaded: false,
    _hidden: false,
    onHealthConsentGuardError(error) {
      guardErrors += 1;
      assert.equal(error, networkError);
    },
  };
  currentPages = [page];
  requestImpl = async () => { throw networkError; };

  assert.equal(await careSession.guardHealthSensitivePage(page, async () => { loads += 1; }), false);
  assert.equal(guardErrors, 1);
  assert.equal(loads, 0);
  assert.match(toasts.at(-1).title, /核验失败/);
  assert.deepEqual(navigations, []);

  requestImpl = async () => ({
    required_health_consent_version: 'health-retry-v1',
    health_consent_current: true,
  });
...
test('健康同意守卫的 401 和精确 428 继续走登录或单独同意导航', async () => {
  resetSession('health-guard-401-token');
  ...
  const unauthorizedPage = {
    ...
    onHealthConsentGuardError() { guardErrors += 1; },
  };
  currentPages = [unauthorizedPage];
  requestImpl = async () => {
  ...
  assert.equal(await careSession.guardHealthSensitivePage(unauthorizedPage, async () => {}), false);
  assert.equal(sessionInvalidations, 1);
  assert.equal(guardErrors, 0);
  assert.equal(relaunchedTo, '/pages/bind-token/index');
  assert.equal(storage.has(SESSION_KEY), false);

  resetSession('health-guard-428-token');
  let consentInvalidations = 0;
  guardErrors = 0;
  const consentPage = {
    _unloaded: false,
    _hidden: false,
    onHealthConsentRequired() { consentInvalidations += 1; },
    onHealthConsentGuardError() { guardErrors += 1; },
  };
  currentPages = [consentPage];
  requestImpl = async () => {
    throw Object.assign(new Error('health consent required'), {
      statusCode: 428,
      code: 'health_sensitive_consent_required',
      data: { required_health_consent_version: 'health-guard-v2' },
    });
  };

  assert.equal(await careSession.guardHealthSensitivePage(consentPage, async () => {}), false);
  assert.equal(consentInvalidations, 1);
  assert.equal(guardErrors, 0);
  assert.equal(navigations.at(-1), '/pages/health-consent/index?required_version=health-guard-v2');
  assert.equal(storage.get(SESSION_KEY).token, 'health-guard-428-token');
});
