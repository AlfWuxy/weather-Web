...
test('健康筛查提交错误持久可读并区分天气、网络和会话状态', async () => {
  const answers = {
    outdoor_exposure: 'low',
    symptom_level: 'none',
    hydration: 'good',
  ...
  const scenarios = [
    {
      error: Object.assign(new Error('不应展示的上游天气缺失细节'), {
        statusCode: 503,
        code: 'weather_snapshot_unavailable',
      }),
      kind: 'weather-unavailable',
      title: /天气数据待更新/,
    },
    {
      error: Object.assign(new Error('不应展示的上游天气过期细节'), {
        statusCode: 503,
    ...
    {
      error: Object.assign(new Error('request:fail timeout bearer-sensitive-value'), {
    ...
    {
      error: Object.assign(new Error('invalid_token internal-sensitive-value'), {
        statusCode: 401,
        code: 'invalid_token',
      }),
      kind: 'session',
  ...
  for (const scenario of scenarios) {
    authApiImpl = async () => { throw scenario.error; };
    const definition = loadPage('../pages/health-assessment/index');
    const page = makePage(definition, {
      pairId: 7,
      contextReady: true,
    ...
    assert.equal(page.data.busy, false);
    assert.equal(page.data.submitError.kind, scenario.kind);
    assert.match(page.data.submitError.title, scenario.title);
    assert.deepEqual(page.data.answers, answers);
    assert.equal(page.data.completedCount, 5);
    assert.doesNotMatch(JSON.stringify(page.data.submitError), /上游|bearer-sensitive|internal-sensitive/);
    assert.match(lastToast.title, /本次未保存/);
  }

  const view = fs.readFileSync(path.join(__dirname, '../pages/health-assessment/index.wxml'), 'utf8');
  assert.match(view, /wx:if="\{\{submitError\}\}"[^>]*role="alert"[^>]*aria-role="alert"[^>]*aria-live="assertive"/);
  assert.match(view, /本次筛查未保存/);
  assert.match(view, /wx:if="\{\{submitError\.retry && contextReady\}\}"/);
  assert.match(view, /bindtap="submitAssessment"/);

  const invalidatedDefinition = loadPage('../pages/health-assessment/index');
  const invalidatedPage = makePage(invalidatedDefinition, {
    pairId: 7,
    contextReady: true,
    loading: false,
    busy: true,
  ...
  invalidatedPage._healthMutationKind = 'assessment-submit';
  invalidatedPage.onSessionInvalidated.call(invalidatedPage);
  assert.equal(invalidatedPage.data.submitError.kind, 'session');
  assert.equal(invalidatedPage.data.contextReady, false);
  assert.deepEqual(invalidatedPage.data.answers, {});
  assert.equal(invalidatedPage.data.busy, false);
});

test('健康筛查失败后重试成功会清除持久错误并保存结果', async () => {
  const answers = {
    outdoor_exposure: 'low',
    symptom_level: 'none',
    hydration: 'good',
  ...
  await page.submitAssessment.call(page);
  assert.equal(page.data.submitError.kind, 'weather-stale');
  assert.deepEqual(page.data.answers, answers);

  await page.submitAssessment.call(page);
  assert.equal(attempts, 2);
  assert.equal(page.data.submitError, null);
  assert.equal(page.data.latest.id, 'saved-result');
  assert.deepEqual(page.data.answers, {});
  assert.equal(page.data.completedCount, 0);
  assert.equal(lastToast.title, '筛查已保存');
});
