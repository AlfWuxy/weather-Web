...
test('求助入口明确通知已授权家属且紧急时直接联系', () => {
  for (const file of [
    ...
    const text = read(file);
    assert.match(text, /已授权家属/, `${file} 缺少已授权家属工作台说明`);
    assert.match(text, /120/, `${file} 缺少紧急联系 120`);
    assert.match(text, /沙盒|不发送真实微信/, `${file} 缺少本轮不发真实外部通知边界`);
  }
});
