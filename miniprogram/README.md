...
1.1.0 小程序只提供微信快捷登录。不同微信 OpenID 映射到不同内部账号；公共天气和社区信息为所有账号共享的都昌县聚合数据，家人、健康、用药和行动资料按内部账号隔离。当前不获取手机号，也不提供手动粘贴 Web Token 的入口。
...
社区、避暑资源和 GIS 元数据统一读取 `GET /mp/api/v1/public/community`。约 1.9 MB 的公开 GeoJSON 只在 GIS 页由用户点击后通过同源后端地址下载，不进入小程序包，也不写入本机持久缓存。避暑资源页可在用户逐次确认后单次调用 `wx.getLocation`，仅在页面内计算 GCJ-02 资源点的直线距离；用户坐标不上传、不持久保存，离页即清除，拒绝后可手选社区。

## 上架前检查

1. 在微信公众平台完善小程序名称、类目、服务内容、用户隐私保护指引和服务条款。
2. 确认 `app.json` 的 `__usePrivacyCheck__` 已启用，并在照护登录入口提供隐私协议查看和同意流程。
3. 按实际调用能力申报网络请求、拨打电话、单次位置和原生地图等功能。定位用途只填写“本次按直线距离排列附近避暑资源”，不得申报持续或后台定位。
4. 确认后端证书有效、TLS 配置正常、生产域名已加入 `request` 白名单。
5. 确认后台 30 分钟同步任务、预算上限保护、失败保留旧快照和监控告警均已启用。
6. 用老人常见机型测试大字号、低网速、断网、缓存过期、空数据、服务端错误和重新登录。
7. 分别测试游客路径、照护登录、不同微信账号隔离、退出登录、账号注销、行动清单、社区筛选、定位逐次确认、拒绝定位后的社区手选、原生地图和 GIS 图层切换。
