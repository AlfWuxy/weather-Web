...
1.1.1 小程序提供微信快捷登录和一次性网页账号绑定码入口。不同微信 OpenID 映射到不同内部账号；公共天气和社区信息为所有账号共享的都昌县聚合数据，家人、健康、用药和行动资料按内部账号隔离。小程序不请求微信手机号，也不调用微信手机号接口；网页端可由用户主动填写未短信验证的手机号，只作待验证跨端账号标识，不能用于网页登录或证明号码归属。不同账号填写相同未验证号码不会互相获得资料，也不会阻断一次性绑定码。小程序不提供手动粘贴 Web Token 的入口。

网页用户复验当前密码后生成 8 位一次性绑定码，10 分钟内有效，明文只显示一次，服务器只保存不可逆哈希。连续失败会临时锁定绑定入口。绑定只接受没有照护数据的微信临时占位账号；成功后旧会话失效，占位账号随机化、标记失效并去关联。账号注销必须重新执行一次 `wx.login`，且本次 OpenID 哈希与当前会话绑定身份一致。

公共今日行动使用 120 条预先编写的家庭提醒。热暴露 GIS 的 65+ 表示模型化 65 岁及以上人口比例，LST 表示 Aqua MODIS 白天晴空地表温度，不等于个人暴露、当前气温或医疗风险。小程序地图使用后端聚合数据、端内 Canvas 和微信原生地图，不直接加载高德地图 SDK；网页高德底图和后台待核验候选属于独立边界。
...
社区、避暑资源和 GIS 元数据统一读取 `GET /mp/api/v1/public/community`。约 1.9 MB 的公开 GeoJSON 只在 GIS 页由用户点击后通过同源后端地址下载，不进入小程序包，也不写入本机持久缓存。避暑资源页可在用户逐次确认后单次调用 `wx.getLocation`，仅在页面内计算 GCJ-02 资源点的直线距离；用户坐标不上传至本项目服务器，也不进入本项目持久存储，离页即清除，拒绝后可手选社区。
