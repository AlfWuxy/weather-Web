...
const HELP_POLL_BASE_MS = 5000;
const HELP_POLL_JITTER_MS = 800;

function helpIdempotencyStorageKey(pairId) {
  return `help_idempotency_${pairId}`;
}

function getOrCreateHelpKey(pairId) {
  const storageKey = helpIdempotencyStorageKey(pairId);
  try {
    const existing = wx.getStorageSync(storageKey);
    if (existing && existing.key) return String(existing.key);
  } catch (error) {}
  const key = `help-${pairId}-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  try {
...
function clearHelpKey(pairId) {
  try {
    wx.removeStorageSync(helpIdempotencyStorageKey(pairId));
  } catch (error) {}
}

function isUncertainHelpError(error) {
  const status = Number(error && error.statusCode) || 0;
  const message = String((error && (error.message || error.errMsg || error.code)) || '');
  return status === 0 || status === 408 || status >= 500
    || /timeout|fail|network|abort/i.test(message);
}
...
Page({
  ...
  async loadContext() {
    ...
    try {
      ...
      try {
        const pending = await authApi({ method: 'GET', path: '/mp/api/v1/pending' });
        if (!requestIsActive(this, '_contextRequestId', request)) return;
        const open = ((pending && pending.help_requests) || []).find((item) => Number(item.pair_id) === pairId);
        this.applyOpenHelp(open);
      } catch (error) {
        // 未结列表失败不影响今日行动主路径。
      }
    } catch (error) {
  ...
  applyOpenHelp(open) {
    const pairId = Number(this.data.pairId || 0);
    if (!open) {
      if (this.data.helpStatus === 'resolved' || this.data.helpStatus === 'cancelled') {
        clearHelpKey(pairId);
      }
      return;
    }
    const status = String(open.status || '');
    if (status === 'resolved' || status === 'cancelled') clearHelpKey(pairId);
    this.setData({
      helpRecorded: true,
      helpPendingConfirm: false,
      helpId: open.id || '',
      helpStatus: status,
      helpStatusLabel: open.status_label || HELP_STATUS_LABELS[status] || status,
    });
  },

  stopHelpPoll() {
    this._stopped = true;
    if (this._helpPollTimer) {
      clearTimeout(this._helpPollTimer);
      this._helpPollTimer = null;
    }
  },
  ...
  scheduleHelpPoll() {
    ...
    if (this._stopped || this._unloaded || this._hidden) return;
    const delay = HELP_POLL_BASE_MS + Math.floor(Math.random() * HELP_POLL_JITTER_MS);
    this._helpPollTimer = setTimeout(async () => {
      this._helpPollTimer = null;
      if (this._stopped || this._unloaded || this._hidden) return;
      await this.refreshOpenHelp();
      this.scheduleHelpPoll();
    }, delay);
    if (this._helpPollTimer && typeof this._helpPollTimer.unref === 'function') {
      this._helpPollTimer.unref();
    }
  },

  async refreshOpenHelp() {
    const pairId = Number(this.data.pairId || 0);
    if (!pairId || this._unloaded || this._hidden) return;
    try {
      const pending = await authApi({ method: 'GET', path: '/mp/api/v1/pending' });
      if (this._unloaded || this._hidden) return;
      const open = ((pending && pending.help_requests) || []).find((item) => Number(item.pair_id) === pairId);
  ...
  requestHelp() {
    ...
    wx.showModal({
      title: updating ? '更新求助说明？' : '向已授权家属求助？',
      content: `${updating ? '本操作会更新已保存的求助说明' : '会生成求助编号并通知已授权家属'}。本轮沙盒默认不发送真实微信/短信。紧急情况请拨打 120。`,
      confirmText: updating ? '更新说明' : '发送求助',
      confirmColor: '#b42318',
      success: async (result) => {
        ...
        this.setData({ busyAction: 'help', helpPendingConfirm: false });
        const pairId = Number(this.data.pairId);
        const idempotencyKey = getOrCreateHelpKey(pairId);
        let mutation = null;
        try {
          mutation = trackHealthMutation(
            this,
            authApi({
              ...
              data: {
                note: String(this.data.helpNote || '').trim().slice(0, 300),
          ...
          if (!lifecycleIsActive(this, lifecycle)) return;
          const helpId = String((body && (body.id || body.help_id)) || '');
          const status = String((body && body.status) || 'pending_ack');
          this.setData({
            helpRecorded: true,
            helpPendingConfirm: false,
            helpId,
            helpStatus: status,
            helpStatusLabel: (body && body.status_label) || HELP_STATUS_LABELS[status] || '待家属接收',
          });
          wx.showToast({ title: updating ? '求助说明已更新' : '求助已提交', icon: 'success' });
        } catch (error) {
          if (!lifecycleIsActive(this, lifecycle)) return;
          if (isUncertainHelpError(error)) {
            this.setData({ helpPendingConfirm: true, helpStatusLabel: '结果待确认' });
            wx.showToast({ title: '结果待确认，正在核对', icon: 'none' });
            await this.refreshOpenHelp();
            return;
          }
