...
地点：小程序公共天气仍钉**都昌县**研究点（`116.20,29.27`）。网页加入天气照护必须显式填写老人所在地，不能用家属定位或县默认值顶替。网站「家庭照护」入口是家人档案；`/pairs` 是提醒与短码。未加入天气照护的家人不按家属当前位置判断阈值。网页复制话术仅高温触发。天气不可用时不生成肯定日常建议。合并本分支 ≠ 微信与正式网页已经互通。
