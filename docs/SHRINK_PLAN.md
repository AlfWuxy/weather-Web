...
> 版本：v2.1（2026-09-26）· 状态：已批准，P4 前置安全网已完成 · 基线：`main@60ef21f`
...
| P4 前置安全网 | 场景锁（S1 到 S10）与逐步数据库变化；`secrets` 随机序列固定；冬季时间点（D3） | 见 5.1；`main` 与 PR 零差异，共 2181 个检查点 |
...
- 破坏性测试：改老人入口版社区日统计的待确认计算、改老人入口版短码有效期、去掉行动令牌 `used_at` 写入、改照护人行动记录备注截断，4 处全部被发现，且都只有场景步骤能发现。
...
| **D3** | 行为锁增加冬季时间点 | **已完成**：冬季环境冻结在 2026-01-15，种子日期整体平移，天气缓存为寒冷天气；727 个冬季检查点中有 166 个与夏季不同（例如出现"保暖"建议） |

### 6.1 实施中的发现（只记录，不在本方案内修改）

| 编号 | 发现 | 说明 |
|---|---|---|
| F1 | 普通用户（`user` 角色）可以调用 `/caregiver/pair/create` 创建配对（场景 S6 第 8 步） | `caregiver_pair_create` 只拦截游客，未检查角色；而升级、备用联系人等照护操作都检查角色。若普通注册用户本来就被视为照护人（子女端），则属设计如此。请产品负责人确认 |
