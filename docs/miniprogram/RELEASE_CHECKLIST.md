...
- [ ] 微信平台官方小程序名称精确为“宜老平安”，小程序内适老天气风险与行动提醒服务名称精确为“宜老天气通”。这组关系属于双名称映射，不代表项目或功能页全局改名；如后台官方名称再次变化，先停止冻结并迁移平台名称关系、重新编译和真机验收。
- [ ] 私密表单同时填写 `WECHAT_MINIPROGRAM_NAME=宜老平安` 与 `WECHAT_SERVICE_NAME=宜老天气通`，两个字段没有互换、合并或遗漏。
- [ ] 已在 `WECHAT_FORM_READY=0` 时运行 `finalize-content`；六份正式材料的可见正文均同时出现平台名和服务名，每份材料各有唯一状态 marker、平台名 marker 与服务名 marker，隐私/协议材料的日期与隐私版本 marker 唯一且一致，正文不再出现“候选”。六份材料的正式 marker 总数为 26 个：6 个状态、6 个平台名、6 个服务名、5 个生效日期和 3 个隐私版本。
- [ ] 双名称材料通过人工复核和全量测试并形成干净 commit 后，已重新运行 `record-freeze`；私密表单记录的目标 commit、候选 `1.1.1` 和六项 Git HEAD blob SHA-256 与当前 HEAD 一致，两个正式门禁仍保持关闭。
...
- [ ] 账号后台以“宜老平安”填写小程序名称；简介和审核说明明确“小程序内提供宜老天气通服务”，图标、服务类目和版本号已同步填写。
...
- [ ] 平台声明 `wx.setClipboardData` 仅在用户点击后写入提醒或公共纳凉点信息，只写不读且复制内容不上传。
- [ ] 平台按实际处理声明 `wx.login`/OpenID 哈希、账号与会话、家人档案与健康字段、老人码、短码明文及不可逆哈希、固定枚举产品事件、基础结构化请求日志、必要安全限流，以及用户逐次确认后的单次位置；当前小程序不收集的第三方推送接收标识、昵称头像、微信手机号和订阅消息未被误报为已收集。网页端用户主动填写的未短信验证手机号已作为独立 Web 账号字段披露。
- [ ] 单次位置只在避暑资源页面实例内用于 GCJ-02 资源点直线距离排序，不上传至本项目服务器、不写入本项目持久存储、不进入产品事件、日志、分享参数或公共缓存；页面隐藏或离开后清除，拒绝或失败后仍可手选社区。
- [ ] 受限结构化请求日志只记录服务端随机请求编号、方法、经过凭据替换的路径、接口、状态和耗时，其他正式应用日志只保留事件类型、模块、级别、函数和代码行；全局过滤器已用 sentinel 证明会丢弃原始消息参数、异常正文和 traceback，全部运行日志不记录请求或响应正文、查询参数、IP、User-Agent、请求头、SQL 参数、会话 token 或用户坐标。
- [ ] 正式环境第三方错误监控和性能追踪保持关闭；配置校验及应用启动已证明非空 `SENTRY_DSN`、非零 tracing 或启用默认 PII 会 fail closed。
- [ ] 正式站点 Nginx server 块已显式声明且只声明 `access_log off` 与 `error_log /dev/null crit`，部署预检通过；切换后的验证窗口内没有生成新的站点访问日志或请求错误明细。
- [ ] systemd 252 的 journald 没有显式容量或时间覆盖，`MaxRetentionSec=0`；systemd journal 当前没有固定保存天数，按磁盘容量边界和日志量自动轮转覆盖。单个文件默认 `MaxFileSec=1 month`，默认容量目标为文件系统 10% 且上限 4 GiB，默认至少保留 15% 空间且该值上限 4 GiB，2026-07-21 核验时实际占用约 2.9 GiB；已向审核材料明确这不是固定天数保留政策。
...
- [ ] 1.1.1 小程序界面提供微信快捷登录和一次性网页账号绑定码入口，不请求微信手机号，也不提供备用 Web Token 输入；不同 OpenID 的私有资料隔离与同一 OpenID 的账号复用已由端到端测试覆盖。
- [ ] 网页手机号只能由用户主动填写，未发送短信验证码且 `phone_verified_at` 为空；只作待验证跨端账号标识，1.1.1 不能用于网页登录或证明号码归属；相同未验证号码不互相泄露资料，也不阻断一次性绑定码。
- [ ] 网页复验当前密码并确认跨端隐私说明后才生成 8 位一次性绑定码；明文只展示一次，10 分钟内有效，数据库、产品事件和运行日志只保留或处理必要哈希与状态，不记录明文。
- [ ] 同一微信身份连续 5 次绑定失败后锁定 10 分钟；锁定期间不继续匹配绑定码，成功后失败计数清零。
- [ ] 绑定只允许空白微信临时占位账号；已有照护数据时拒绝自动合并。成功后旧会话撤销并轮换，临时占位账号随机化、标记失效，账号身份和事件引用去关联。
- [ ] 账号注销必须由当前小程序会话提交 fresh `wx.login` 临时代码；本次 OpenID 哈希与当前身份不一致时拒绝注销，成功后手机号、绑定挑战、微信身份、会话和 owner 数据按文案清理或匿名化。
...
- [ ] 平台按必要安全限流声明临时 IP 哈希处理；1.1.1 数据库审计日志关闭，不把 IP 哈希或 User-Agent 写入应用审计表，匿名账号注销占位记录不含网络标识。
...
- [ ] “添加家人”首次 `onLoad/onShow` 并发不会永久 loading；健康同意核验超时、网络失败或 5xx 会显示持久错误，重试仍先经过健康同意门禁。
- [ ] 避暑资源定位每次先显示自定义用途确认；允许后只调用一次 `getLocation`，拒绝或失败后手选社区，离页清除，缺坐标资源明确显示“距离待维护”并排在有距离资源之后。
- [ ] 小程序避暑资源只使用项目维护的 GCJ-02 点位并在点击后调用微信原生地图，不直接加载高德地图 SDK；网页高德底图与后台高德地理编码候选单独披露，候选经过人工核对后才可进入公开资源列表。
- [ ] GIS 延迟加载，不阻塞首屏；加载失败仍能查看方法和统计摘要。
- [ ] GIS 文案明确 65+ 是正人口支持网格中的模型化 65 岁及以上人口比例，LST 是 Aqua MODIS 白天晴空地表温度；不得描述为个人年龄、个人暴露、当前气温、体感温度、临床风险或法定边界。
- [ ] 今日行动模板库精确包含 120 条唯一预写提醒；按都昌县日期、公共风险等级、天气标签和受众稳定选择，文案不声称个性化医疗生成、消息已送达或风险已消除。
...
- [ ] 1.1.1 双名称正式材料冻结后已重新编译并重新上传；上传包版本号、构建标识、提交说明、截图、回滚 commit 和新上传回执均已记录，1.0.0 与 1.1.0 回执只保留为历史证据。
