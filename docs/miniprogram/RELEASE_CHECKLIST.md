...
- [ ] 平台按实际处理声明 `wx.login`/OpenID 哈希、家人档案与健康字段、WxPusher UID、固定枚举产品事件和必要安全限流；代码未调用的定位、昵称头像、手机号和订阅消息未被误报为已收集。
...
- [ ] WxPusher 默认关闭；迁移后历史开启状态已统一关闭，重新开启时前端展示完整第三方传输范围且后端强制收到当前说明版本和 UTC 同意时间；缺失、过期或无时间回执时发送端 fail closed。
- [ ] WxPusher 关闭后同意回执保留至账号注销；说明版本变化后必须重新同意，账号注销会清空 UID、回执和投递状态。
- [ ] 已核对服务商公开的 7 天消息/状态保存口径和“删除详情不删除已推送消息”边界；当前有效隐私政策或数据处理条款 URL 已写入平台指引和私有发布确认单。缺少 URL 时保持通道关闭并阻断全功能提交。
...
- [ ] WxPusher 默认关闭；开启时已经取得当前用户对 UID 和第三方传输范围的当前版本明确同意，并保存版本与 UTC 时间。
- [ ] 推送正文不含老人姓名和细粒度地址，推送开关、成员级权限和隐私级别均生效。
- [ ] 推送链接使用最长 7 天随机持有者令牌；GET/HEAD 不写送达确认，首次有效 POST 只记录一次，页面明确无法核验实际点击者身份且不得转发。
- [ ] 送达确认时间和自动确认标记满 30 天后清空；防重复投递状态和人工复核记录保留至账号注销。
