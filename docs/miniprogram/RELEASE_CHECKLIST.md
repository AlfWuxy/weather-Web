...
- [ ] 双名称材料通过人工复核和全量测试并形成干净 commit 后，已重新运行 `record-freeze`；私密表单记录的目标 commit、候选 `1.1.0` 和六项 Git HEAD blob SHA-256 与当前 HEAD 一致，两个正式门禁仍保持关闭。
...
- [ ] 平台按实际处理声明 `wx.login`/OpenID 哈希、账号与会话、家人档案与健康字段、老人码、短码明文及不可逆哈希、固定枚举产品事件、基础结构化请求日志、必要安全限流，以及用户逐次确认后的单次位置；当前版本不收集的第三方推送接收标识、昵称头像、手机号和订阅消息未被误报为已收集。
- [ ] 单次位置只在避暑资源页面实例内用于 GCJ-02 资源点直线距离排序，不上传服务器、不写入持久存储、不进入产品事件、日志、分享参数或公共缓存；页面隐藏或离开后清除，拒绝或失败后仍可手选社区。
...
- [ ] 1.1.0 小程序界面只提供微信快捷登录，不提供备用 Web Token 输入；不同 OpenID 的私有资料隔离与同一 OpenID 的账号复用已由端到端测试覆盖。
...
- [ ] 平台按必要安全限流声明临时 IP 哈希处理；1.1.0 数据库审计日志关闭，不把 IP 哈希或 User-Agent 写入应用审计表，匿名账号注销占位记录不含网络标识。
...
- [ ] 1.1.0 双名称正式材料冻结后已重新编译并重新上传；上传包版本号、构建标识、提交说明、截图、回滚 commit 和新上传回执均已记录，1.0.0 回执只保留为历史证据。
