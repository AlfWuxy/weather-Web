...
- WxPusher 默认关闭，用户填写 UID、阅读完整第三方传输范围并在一般隐私同意之外单独勾选后才可开启；每次重新开启都需要当次明确同意。传输字段仅限用于识别推送目标的 WxPusher UID、都昌县级预警标题与正文及 7 天内有效的点击链接，不得包含老人姓名、健康筛查、健康日记、用药记录、家庭地址或细粒度位置。打开页面本身不会记录；只有用户主动点击“我已看到这条提醒”后，服务端才首次记录一次送达确认。`PUSH_TRACKING_LINK_TTL_DAYS` 默认且最长为 7 天，生产环境只允许缩短。
