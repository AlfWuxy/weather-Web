...
2. 从个人主体当时可选的正式类目中，选择能够覆盖候选包完整真实功能的全部必要类目。核对范围必须包含公开天气、七日预报、官方预警、社区脆弱性、避暑资源、热暴露 GIS、本机公共行动清单，以及登录后的家人档案、五项健康筛查、200 字症状与 500 字备注健康日记、用药记录、家人行动确认、求助、复盘和可选 WxPusher 推送。禁止为了使用更窄类目而省略、改名或隐藏已存在的功能。页面出现额外许可证、机构证明或企业材料时停止，保持 `WECHAT_CATEGORY_CONFIRMED=0`。
...
`WECHAT_FORM_READY=0` 时的表单校验只用于本地预览，不会读取 AppID、AppSecret 或 WxPusher token，也不会生成微信 OpenID pepper 或会话密钥。只有同一次 `0600` 验证快照同时满足 `DEPLOY_REQUIRE_WECHAT_READY=1` 与 `WECHAT_FORM_READY=1`，发布脚本才允许下发这些正式配置。
...
- 平台数据类型按代码实际处理逐项声明：`wx.login` 产生的 OpenID 哈希、家人档案与健康字段、WxPusher UID、固定枚举产品事件和必要安全限流。小程序未调用个人定位、昵称头像、手机号或订阅消息，不得为了“看起来完整”而误勾这些数据类型。
- 一般隐私同意和 WxPusher 单独同意都必须保存当前说明版本与 UTC 时间；WxPusher 缺失、过期或无时间回执时，设置保存与发送端都 fail closed。关闭通道可清空 UID，同意回执保留至账号注销以证明授权历史；说明变化后重新同意。
- WxPusher 服务商当前有效的隐私政策或数据处理条款 URL、公开 7 天消息/状态保存口径及“删除详情不删除已推送消息”边界必须在正式启用前复核并写入私有确认单。个人主体后台没有匹配的数据类型、第三方共享项或服务类目，或该 URL 无法核验时，保持通道关闭并停止全功能提交。
...
- WxPusher 默认关闭，用户填写 UID、阅读完整第三方传输范围并在一般隐私同意之外单独勾选当前说明后才可开启；服务端保存说明版本和 UTC 同意时间，说明变化后必须重新同意。传输字段仅限用于识别推送目标的 WxPusher UID、都昌县级预警标题与正文及 7 天内有效的随机持有者链接，不得包含老人姓名、健康筛查、健康日记、用药记录、家庭地址或细粒度位置。打开或预览链接不会写入送达确认，服务器仍可能生成必要的访问安全日志；只有持有链接的人主动 POST 确认后才首次记录一次，页面无法核验实际点击者身份，链接不得转发。确认时间和自动确认标记满 30 天后清空，防重复投递状态和人工复核记录保留至账号注销。`PUSH_TRACKING_LINK_TTL_DAYS` 默认且最长为 7 天，生产环境只允许缩短。
- `sending` 超时、`failed` 和 `uncertain` 投递进入管理员“试点数据看板”的推送人工复核队列。管理员根据用户主动确认和供应商结果选择“确认送达”“确认未送达”或“允许重试一次”。未完成复核前禁止自动重发，复核动作和操作者写入审计记录；用户后续确认不得覆盖已经完成的人工复核结论。
