...
本轮已修的用户路径问题：登录用户名大小写与锁定桶对齐；用药提醒按 `time_of_day` 到点出现（不再要求慢病+在线天气）；健康评估筛查回填；家庭档案「慢阻肺」与 COPD 同义；社区缺测坐标标为估算；天气 API 不再把 Open-Meteo 的 AQI=0 当成实测；照护页区分「20:00 前确认」与「2 小时未确认转备用联系人」，卡片同时显示两套倒计时；`/entry` 老人入口改为照护码确认，首页仍进极简模式。慢病评估改为 PRG，刷新保留上次结果；和风实况写入热浪/寒潮连续天数；试点 CSV 改为带 CSRF 的 POST；个人设置可撤销 API 凭证。7 天健康预测在滞后观测不足或门诊基线缺失时不再编造就诊负担百分比；无当天污染物预报时，7 天暴露分只按气温、湿度和热夜计分，不把当前实况 PM 或默认 AQI 50 算进「风险 N」。健康评估在社区 VI/负担为代理值或缺测 AQI/湿度时不把这些数字编进个人风险分；未填写年龄或五项筛查未答完时不评估。无模型时 `/ml-prediction` 不提供生成入口；有模型时也不再用 65 岁填年龄。慢病评估同样要求个人设置已有年龄。Open-Meteo 缺湿度不再写成 60。API 层慢病/ML 在 JSON 和资料都缺年龄时返回 400，不再用 40/50 岁顶上。ML 个人/社区预测在未传天气时走和风实况，mock/demo 返回 503，不再用 20°C、湿度 70、AQI 50 顶上；传入气温但未传湿度/AQI/风速时保留缺测，不填 70/50/2.5。未传日照时长不再填 20000 秒；社区预测未传人口/老龄率不再填 100/0.2。健康评估服务缺年龄或缺气温时停止计算，不再用 45 岁或 20°C 顶上。Open-Meteo 日预报缺最高/最低气温时跳过该日，不再填 25/15°C；短临小时缺气温时跳过，不再填 0°C。7 天健康预测缺某日气温时停止就诊负担计算，不再填 15°C。今日页无行动提示时不再编造 1.5L 补水或测血压/降压药。Open-Meteo 未知天气代码不再写成「多云」或缺测代码写成「晴」。极端天气识别不再把缺测气温/湿度/风速/AQI 当成 0。ML 预测服务缺年龄或缺气温时停止计算，天气特征向量不再用 15°C/湿度 70 等训练集缺省值顶上。7 天复合暴露缺气温时停止计分，不再填 20°C；缺夜间最低气温不再用「白天减 4°C」编造热夜，缺湿度不再填 60。社区档案缺人口时保留空值，不再填 100 人进入排名。社区绿地、热岛和医疗可达性缺测时不计入脆弱性指数和 SVI，不再写入稳定代理值。小程序新建老人与网页一样校验性别。今日页主按钮排出照护工作台 → 记录今天是否做到 → 附近避暑点 → 7 天趋势。避暑页温度建议和页脚从 `cooling_page.json` 的 `bands`/`footer` 读取。和风实况缺天气状况不再写成「晴」，日预报缺风速不再填 3.0。热风险缺夜间最低温不再复用当前气温。老人模式无行动清单时不再编造「身边放一杯水」。照护话术只列出已登记慢病名称，不再追加感冒/胸闷临床句。DLNM `calculate_rr` 缺气温时停止计算，不再填 20°C。求助文案区分是否开通推送。每日行动提示已抽到 `data/content/daily_action_tips.json`；照护提醒话术已抽到 `data/content/caregiver_tip_scripts.json`；避暑页导语和温度分档已抽到 `data/content/cooling_page.json`；指标说明已抽到 `data/content/metric_explanations.json`。今日页/长者页标题和空清单已抽到 `data/content/dashboard_copy.json`；7 天「本周建议」已抽到 `data/content/forecast_week_tips.json`，改为照护口吻，不再写增派医护。社区风险页「管控建议」已改为「优先行动」，文案从 `data/content/community_action_tips.json` 读取，不再写增派医疗资源。社区公平性「优先干预」行动同样从该 JSON 的 `equity` 读取，不再写临时接诊/病例核验。未知热风险等级不再套用低风险动作清单，页面显示「风险未知」。社区地图缺测 `risk_index` 不再填 55 或画成 0 分低风险。多源预报缺最高/最低气温时不再用 8°C 温差编造昼夜气温。和风与 Open-Meteo 都失败时 7 天预报返回空列表，不再把未标记的模拟天当实测缓存。社区工作台入口不再混用「社区看板」称呼。7 天 API 的 `role_action_cards` 仍保留 `resident`/`doctor`/`community` 键，文案已改为照护口吻并从 `forecast_week_tips.json` 的 `role_cards` 读取。健康评估缺慢病专项分时不再按 30 分融合，页面显示资料不足。健康评估行动建议已抽到 `data/content/health_assessment_tips.json`；慢病规则文案已抽到 `data/content/chronic_recommendation_copy.json`（触发条件仍在 Python）。连续高风险通知和紧急分诊改为联系家属，不再写村医排班。小程序 elders/alerts 与网页共用 `heat_weather_available`（真实和风 + 气温/最高/最低/湿度齐全），缺湿度不再报高温或寒潮。小程序话术温度与网页一样四舍五入，并附同一句慢病提示；预警页没有 `pair_id` 时显示「缺少照护对象」，不再写成「暂无官方预警」。社区日基线门诊按实际人口估算，不再把不足 10 人抬成 10 人；无效天气 RR 或门诊基线不再按 1.0/5 顶上。7 天复合暴露在 PM2.5 与 AQI 都缺测时不再把默认 AQI 50 写成已用值或 PM2.5 代理。家庭成员未填年龄时风险显示「风险未知」，不再用 15 分底分编成低风险。未训练的 DLNM 不再按偏离 20°C 编造 RR；已训练但缺最适温度时也不再用 20°C/23°C 顶上，调用方停止计算而不是把 RR 当成 1.0。热指数缺湿度不再改用气温，风险等级与分数一起留空；同步任务也不再把未知热等级写成低风险。DLNM 滞后气温缺测不再用当天气温顶上；社区/健康评估遇到缺日滞后序列时不再错位补齐；7 天滞后剖面缺测不再填 15°C。用药提醒、家人阈值和慢病建议在温度或 AQI 缺测时停止比较，不再把空值当成 0°C 或空气质量达标。小程序照护对象列表无预警时与网页一样显示「暂无预警」；话术页去掉「可信节点」，缺少 pair 时显示「缺少照护对象」。普通用户导航、今日页和 AI 悬浮不再露出研究工具；`/ai-qa`、`/chronic-risk`、`/ml-prediction`、`/annual-report` 直链对非管理员返回权限不足。社区综合风险在该村没有 SIR 时不再用 50 分位顶上历史负担。DLNM 缺温度分位数时不再用 35/32/5/2°C 或热夜 22°C 标记极端天气。后台新增/编辑社区不再写入医疗可达性 60、环境质量 70 的假设脆弱性指数。家人详情和健康日记在湿度缺测时不再对 None 做 `|round` 而 500；家人页与照护/小程序一样用 `heat_weather_available`。家庭成员年龄与小程序同为 1–150。健康评估服务缺筛查不再填「中等/良好」；AQI/湿度计分缺测不再填 50/60。小程序预警温度与列表一样四舍五入；列表页提示打卡和避暑在网页完成；编辑性别改为选项。
...
| 内容写死在代码里 | 🟢 低 | 照护提示、话术、避暑分档、指标解释、今日/长者标题与空清单、7 天建议、社区优先行动、健康评估建议、慢病/ML 文案已在 `data/content/`。触发阈值仍在 Python。 |
| 研究工具已退出照护主路径 | 🟢 低 | 普通用户导航/今日页/AI 悬浮已隐藏；`/ai-qa` `/chronic-risk` `/ml-prediction` `/annual-report` 直链对非管理员 302。健康日记仍可直链（照护记录）。仓库没有 `models/*.pkl` |
...
1. **一条主路径**：今天风险 → 提醒家人 → 记录是否做到 → 附近避暑。Web 主导航保持「今天 / 照护 / 7 天预报 / 社区风险 / 老人模式」；研究分析（AI 提问、慢病评估、疾病预测、年度报告、AI 悬浮）只留在管理员，路由已拦截。健康日记仍可直链，不在普通导航。小程序保持薄客户端：档案、预警、话术、解除照护；打卡和避暑在网页完成（列表页与话术页均已提示）。
...
5. **双端同一领域模型**：Web 家庭成员与小程序 elders 共用同一套档案字段，年龄 1–150，性别用选项而不是自由文本。热风险天气门槛已抽到 `core/weather.py` 的 `heat_weather_available`；家人列表/详情缺湿度不再当天气可用去 `|round`。话术与预警温度取整、无预警文案、缺 pair 空态已对齐。小程序 `DELETE /mp/api/v1/elders/<pair_id>` 与网页删除家人共用 `unbind_family_member_for_caregiver`。打卡和避暑仍只在网页，列表页与话术页已提示到网页完成。
...
- **下一轮产品**：可选把小程序打卡/避暑做成网页深链而不是再做一套原生页；社区表若要真实 VI，给后台表单加医疗可达性/环境质量字段，而不是再写假设值。后台社区图缺测 VI 仍画成 0 仅用于管理图表。
