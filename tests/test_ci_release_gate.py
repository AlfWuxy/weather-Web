...
def test_ci_uses_pinned_actions_and_locked_dependencies() -> None:
    ...
    assert workflow.count(f"uses: actions/checkout@{CHECKOUT_SHA}") == 2
    assert workflow.count(f"uses: actions/setup-python@{SETUP_PYTHON_SHA}") == 2
...
def test_ci_release_proof_is_push_only_and_depends_on_both_test_groups() -> None:
    workflow = _read(CI_WORKFLOW)
