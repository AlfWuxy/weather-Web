...
if command == 'cat':
    raise SystemExit(0 if marker('exists').exists() else 1)
...
"""
    ...
    for unit in ('case-weather.service',) + NEW_TIMER_UNITS + LEGACY_UNITS:
...
def test_success_switches_release_only_after_migration_and_health(tmp_path):
    ...
    assert (transaction['state_dir'] / '.env').read_text(encoding='utf-8').endswith(
        'RELEASE_VALUE=new\n'
    )
    ...
    for unit in ('case-weather.service',) + NEW_TIMER_UNITS:
