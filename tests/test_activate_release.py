...
if action == 'reserve':
    ...
    elif mode == 'error-after-reserve':
        response = {{'outcome': 'ambiguous-after-reserve', 'exit_code': 74}}
    else:
        response = {{'outcome': 'acquired', 'exit_code': 0}}

    # 模拟等待期间旧 owner 的合法计数变化，验证预算基线在取得租约后冻结。
    ...
    active_units = [
        ...
        if any(
            (state / f'{{unit}}.{{kind}}').exists()
            for kind in ('active', 'activating', 'reloading', 'deactivating')
        )
    ]
    receipt_root = Path(
    ...
    lease_environment = Path(os.environ['CASE_WEATHER_ENV_FILE'])
    gate_values = [
        line.split('=', 1)[1]
        for line in lease_environment.read_text(encoding='utf-8').splitlines()
        if line.startswith('QWEATHER_NETWORK_NOT_BEFORE_EPOCH=')
    ]
    transactions = sorted(
        Path(
            {str(transaction['state_dir'] / 'backups' / 'deploy-transactions')!r}
        ).iterdir()
    )
    ...
    exit_code = int(response.get('exit_code', 70))
    if exit_code == 0 or mode in {{'error-after-reserve', 'pause-after-reserve'}}:
...
""",
    ...
    transaction.update({
        ...
        'formal_lease_reservation_count': lease_reservation_count,
        'formal_lease_renewal_count': lease_renewal_count,
        'formal_lease_release_count': lease_release_count,
        'formal_lease_observation_file': lease_observation_file,
        'formal_lease_action_log': lease_action_log,
    })
...
def _assert_formal_lease_wait_is_pre_irreversible(events):
    assert events
    for event in events:
        assert 'case-weather.service' in event['active_units']
        assert event['all_scheduler_stops_seen'] is False
...
def _assert_formal_lease_failure_rolled_back(
    transaction,
    counter_file,
    *,
    expected_rollback='pre-mutation\n',
):
    ...
    rolled_back = transaction_dir / 'ROLLED_BACK'
    assert rolled_back.read_text(encoding='utf-8') == expected_rollback
    ...
    assert not (transaction_dir / 'formal-smoke-lease.journal').exists()
...
def test_formal_smoke_waits_for_expiring_lease_before_production_mutation(tmp_path):
    transaction = _prepare_transaction(tmp_path)
    _staged_text, counter_file = _configure_formal_smoke(transaction)
...
def test_formal_smoke_lease_is_reserved_before_scheduler_quiescence(
    tmp_path,
):
    transaction = _prepare_transaction(tmp_path)
    _staged_text, counter_file = _configure_formal_smoke(transaction)
    ...
    assert len(events) == 1
    _assert_formal_lease_wait_is_pre_irreversible(events)
    assert transaction['formal_lease_action_log'].read_text(
        encoding='utf-8'
    ).splitlines() == ['reserve', 'release']
    _assert_formal_lease_failure_rolled_back(
        transaction,
        counter_file,
        expected_rollback='success\n',
    )
